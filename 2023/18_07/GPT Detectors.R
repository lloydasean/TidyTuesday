...
#was detector prediction correct?
...
detector_accuracy %>% ggplot(aes(y=percent_correct,x=reorder(detector,percent_correct)))+
  ...
  scale_fill_gradient("Accuracy",low = "#ff9696",high = "#c8ff96",labels=scales::percent)+
  labs(title="GPT Detector Accuracy",x="Detector",y=NULL)+
...
model_undetectable %>% ggplot(aes(y=percent_undetected,x=model))+
  geom_col(width=0.5)+
  scale_y_continuous(labels=scales::percent)+
  labs(title="GPT3 vs GPT4",x="Model",y=NULL, subtitle="Percentage of essays written by GPT that go undetected")
...
false_positives %>% ggplot(aes(y=percent_correct,x=detector,fill=native))+
  ...
  scale_y_continuous(labels=scales::percent)+
  labs(fill="Native English Speaker",title="GPT Detector Accuracy",subtitle="Native vs Non-Native English Spekaers",x="Detector",y=NULL)+
  theme(axis.text.x=element_text(angle=50,vjust=0.6))
